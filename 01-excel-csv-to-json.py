    ...
    def transform_values(values):
        ...
        for column, value in values.items():
            if isinstance(value, dict):
                ...
                transformed_values["{}".format(count)] = {"キー": column, "値": transform_values(value)}
            else:
                transformed_values["{}".format(count)] = {"キー": column, "値": value}
            count += 1
...
def createB_T(C_T_json):
    def createB_T_recursive(values):
        ...
        for i, (key, value) in enumerate(values.items(), start=1):
            if isinstance(value, dict):
                # Recursive call for nested dictionaries
                transformed_values[i] = {"キー": key, "値": createB_T_recursive(value)}
            else:
                transformed_values[i] = {"キー": key, "値": value}
        return transformed_values
